...
export default
{
  ...
  setup(props,ctx ){
    ...
    const beginUpload=(file)=>{
      ...
      axios.post(apiRoot+'/FileService/File/UploadItems', formData, config).then(
      (response) =>
      {
    ...
    const upload=function(){      
      ...
      let fileReads = new FileReader();
        ...
        fileReads.onload=async function(){
          ...
          //检查是否在服务器端有这个文件
          /* const {data} = await axios.get(`${apiRoot}:5001/Uploader/FileExists?fileSize=${fileSize}&sha256Hash=${hash}`);
          if(data.isExists)
          {
